(ns my-clojure-project.db
  ...
  (:import [org.apache.jena.tdb2 TDB2Factory]
           [org.apache.jena.query QueryFactory QueryExecutionFactory QuerySolutionMap]
...
(defn execute-sparql-select
  ([query-string]
   (execute-sparql-select query-string {}))
  ([query-string params]
   (let [dataset (get-dataset)]
     (try
       ...
       (let [model (.getDefaultModel dataset)
             ...
             param-map (reduce-kv (fn [m k v]
                                    (.add m (name k) v)
                                    m)
                                  (QuerySolutionMap.)
                                  params)]
         (with-open [qexec (QueryExecutionFactory/create query model param-map)]
