(ns rwclj.vcard
  ...
  (:import [org.apache.jena.rdf.model Model ModelFactory ResourceFactory] ; Added Model
           ...
           ; [org.apache.jena.tdb2 TDB2Factory] Removed, using db namespace
           [java.util UUID]))
...
(defn generate-person-uri [vcard-data]
...
(defn vcard->rdf [vcard-data person-uri model]
...
;; (defn store-vcard-rdf! [person-uri model] ; Function moved to db.clj
;;   "Store the RDF model to TDB2"
...
;;       (catch Exception e
;;         (.abort dataset)
;;         (log/error "Failed to store vCard RDF:" (.getMessage e))
;;         (throw e))
;;       (finally
;;         (.close dataset)))))

(defn validate-vcard [vcard-text]
...
;; HTTP handlers
(defn import-vcard-handler [request]
        ...
        (let [vcard-text body]
          (if (validate-vcard vcard-text)
            (let [vcard-data (parse-vcard vcard-text)
              ...
              (db/store-rdf-model! model) ; Use db namespace
...
(defn test-vcard-import []
  ...
  (let [vcard-data (parse-vcard sample-vcard)
    ...
    (db/store-rdf-model! model) ; Use db namespace
    (log/info "Test vCard imported for person:" person-uri) ; Use logging
