...
describe('App Component', () => {
  ...
  test('media type dropdown has correct options', () => {
    ...
    expect(screen.getByRole('option', { name: 'Book (EPUB)' })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: 'Media (MP4)' })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: 'Music (MP3)' })).toBeInTheDocument();
  });
  ...
  test('successful file upload for book', async () => {
    ...
    const file = new File(['test content'], 'test.epub', { type: 'application/epub+zip' });
    await userEvent.upload(fileInput, file);
    await userEvent.selectOptions(mediaTypeSelect, 'book');
    await userEvent.click(uploadButton);
  ...
  test('successful file upload for media', async () => {
    fetch.mockResolvedValueOnce({ ok: true });
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/media', {
        method: 'POST',
        body: expect.any(FormData),
      });
    });
  ...
  test('successful file upload for music', async () => {
    fetch.mockResolvedValueOnce({ ok: true });
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/music', {
        method: 'POST',
        body: expect.any(FormData),
      });
    });
  ...
  test('handles file upload network error', async () => {
    ...
    await waitFor(() => {
      expect(alert).toHaveBeenCalledWith('An error occurred while uploading the file.');
    });
  });

  // Successful data submission tests
  test('successful data submission for event', async () => {
    fetch.mockResolvedValueOnce({ ok: true });
    ...
    await userEvent.type(dataTextarea, 'Test event data');
    await userEvent.selectOptions(dataTypeSelect, 'event');
    await userEvent.click(submitButton);
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/events', {
        ...
        body: 'Test event data',
      });
    });
    expect(alert).toHaveBeenCalledWith('Data submitted successfully!');
  });

  test('successful data submission for person', async () => {
    fetch.mockResolvedValueOnce({ ok: true });
    ...
    await userEvent.type(dataTextarea, 'Test person data');
    await userEvent.selectOptions(dataTypeSelect, 'person');
    await userEvent.click(submitButton);
    
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/persons', {
        method: 'POST',
        headers: {
          'Content-Type': 'text/plain',
        },
        body: 'Test person data',
      });
    });
    expect(alert).toHaveBeenCalledWith('Data submitted successfully!');
  });

  test('successful data submission for TTL', async () => {
    fetch.mockResolvedValueOnce({ ok: true });
    ...
    await userEvent.type(dataTextarea, 'Test TTL data');
    await userEvent.selectOptions(dataTypeSelect, 'ttl');
    await userEvent.click(submitButton);
    
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/ttl', {
        method: 'POST',
        headers: {
          'Content-Type': 'text/plain',
        },
  ...
  test('handles data submission network error', async () => {
    ...
    await waitFor(() => {
      expect(alert).toHaveBeenCalledWith('An error occurred while submitting the data.');
    });
  });
});
